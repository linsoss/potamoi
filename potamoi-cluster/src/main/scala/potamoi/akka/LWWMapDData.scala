...
trait LWWMapDData[Key, Value](cacheId: String):
  ...
  object op:
    extension (actor: ActorRef[Req])(using cradle: ActorCradle) {
      def get(key: Key, timeout: Option[Duration] = None): AIO[Option[Value]] = actor.askZIO(Get(key, _), timeout)
      def contains(key: Key, timeout: Option[Duration] = None): AIO[Boolean]  = actor.askZIO(Contains(key, _), timeout)
      def listKeys(timeout: Option[Duration] = None): AIO[List[Key]]          = actor.askZIO(ListKeys.apply, timeout)
      def listValues(timeout: Option[Duration] = None): AIO[List[Value]]      = actor.askZIO(ListValues.apply, timeout)
      def listAll(timeout: Option[Duration] = None): AIO[Map[Key, Value]]     = actor.askZIO(ListAll.apply, timeout)
      def size(timeout: Option[Duration] = None): AIO[Int]                    = actor.askZIO(Size.apply, timeout)

      def put(key: Key, value: Value): AIO[Unit]                                    = actor.tellZIO(Put(key, value))
      def puts(kv: List[(Key, Value)]): AIO[Unit]                                   = actor.tellZIO(Puts(kv))
      def remove(key: Key): AIO[Unit]                                               = actor.tellZIO(Remove(key))
      def removes(keys: List[Key]): AIO[Unit]                                       = actor.tellZIO(Removes(keys))
      def removeBySelectKey(filter: Key => Boolean): AIO[Unit]                      = actor.tellZIO(RemoveBySelectKey(filter))
      def update(key: Key, updateValue: Value => Value): AIO[Unit]                  = actor.tellZIO(Update(key, updateValue))
      def upsert(key: Key, putValue: Value, updateValue: Value => Value): AIO[Unit] = actor.tellZIO(Upsert(key, putValue, updateValue))
    }
