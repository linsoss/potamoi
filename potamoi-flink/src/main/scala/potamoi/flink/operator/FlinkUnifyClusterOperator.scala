...
import potamoi.flink.model.deploy.JobSavepointSpec
...
import potamoi.flink.operator.FlinkUnifyClusterOperator.*
...
import potamoi.zios.{repeatWhileWithSpaced, someOrFailUnion}
 ...
 */
trait FlinkUnifyClusterOperator {
...
object FlinkUnifyClusterOperator {
  type KillClusterErr  = (ClusterNotFound | FlinkDataStoreErr | K8sFailure | AkkaErr) with FlinkErr
  type JobOperationErr = (ClusterNotFound | JobNotFound | FlinkDataStoreErr | K8sFailure | FlinkRestErr) with FlinkErr
}

/**
 * Default implementation.
 */
class FlinkUnifyClusterOperatorImpl(
    flinkConf: FlinkConf,
  ...
  override def killCluster(fcid: Fcid): IO[KillClusterErr, Unit] = {
    val ef: IO[KillClusterErr, Unit] = for
      // untrack cluster
  ...
  protected def internalKillCluster(fcid: Fcid, wait: Boolean): IO[FlinkDataStoreErr | ClusterNotFound | K8sFailure, Unit] = {
    if wait then
      k8sOperator.client.deployments
        ...
        .refineToOrDie[FlinkErr.ClusterNotFound | FlinkErr.K8sFailure]
        .unit
  ...
  override def cancelJob(fjid: Fjid): IO[JobOperationErr, Unit] = {
    val ef: IO[JobOperationErr, Unit] =
      for {
  ...
  override def stopJob(fjid: Fjid, savepoint: JobSavepointSpec): IO[JobOperationErr, (Fjid, TriggerId)] = {
    val ef: IO[JobOperationErr, (Fjid, TriggerId)] =
      for {
        restUrl   <- observer.restEndpoint
                       .getEnsure(fjid.fcid)
                       .someOrFailUnion(ClusterNotFound(fjid.fcid))
