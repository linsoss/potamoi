...
class ClusterOverviewMemoryStorage(ref: Ref[mutable.Map[Fcid, FlinkClusterOverview]]) extends ClusterOverviewStorage:
  ...
  def get(fcid: Fcid): IO[DataStoreErr, Option[FlinkClusterOverview]] = stg.get(fcid)
  def listAll: Stream[DataStoreErr, FlinkClusterOverview]             = stg.streamValues
  def put(ov: FlinkClusterOverview): IO[DataStoreErr, Unit]           = stg.put(ov.fcid, ov)
...
class JmMetricsMemoryStorage(ref: Ref[mutable.Map[Fcid, FlinkJmMetrics]]) extends JmMetricsStorage:
  private val stg                                                 = MapBasedStg(ref)
  def get(fcid: Fcid): IO[DataStoreErr, Option[FlinkJmMetrics]] = stg.get(fcid)
  def put(metric: FlinkJmMetrics): IO[DataStoreErr, Unit]       = stg.put(Fcid(metric.clusterId, metric.namespace), metric)
  def rm(fcid: Fcid): IO[DataStoreErr, Unit]                    = stg.delete(fcid)

class TmDetailMemoryStorage(ref: Ref[mutable.Map[Ftid, FlinkTmDetail]]) extends TmDetailStorage:
  private val stg                                                = MapBasedStg(ref)
  def put(tm: FlinkTmDetail): IO[DataStoreErr, Unit]           = stg.put(tm.ftid, tm)
  def putAll(tm: List[FlinkTmDetail]): IO[DataStoreErr, Unit]  = stg.putAll(tm.map(tm => tm.ftid -> tm).toMap)
  def rm(ftid: Ftid): IO[DataStoreErr, Unit]                   = stg.delete(ftid)
  def rm(fcid: Fcid): IO[DataStoreErr, Unit]                   = stg.deleteByKey(_.fcid == fcid)
  def get(ftid: Ftid): IO[DataStoreErr, Option[FlinkTmDetail]] = stg.get(ftid)
  def list(fcid: Fcid): IO[DataStoreErr, List[FlinkTmDetail]]  = stg.getByKey(_.fcid == fcid)
  def listAll: Stream[DataStoreErr, FlinkTmDetail]             = stg.streamValues
  def listTmId(fcid: Fcid): IO[DataStoreErr, List[Ftid]]       = stg.getPartByKey(_.fcid == fcid, _.ftid)
  def listAllTmId: Stream[DataStoreErr, Ftid]                  = stg.streamValues.map(_.ftid)

class TmMetricMemoryStorage(ref: Ref[mutable.Map[Ftid, FlinkTmMetrics]]) extends TmMetricStorage:
  private val stg                                                 = MapBasedStg(ref)
  def put(metric: FlinkTmMetrics): IO[DataStoreErr, Unit]       = stg.put(Ftid(metric.clusterId, metric.namespace, metric.tmId), metric)
  def rm(ftid: Ftid): IO[DataStoreErr, Unit]                    = stg.delete(ftid)
  def rm(fcid: Fcid): IO[DataStoreErr, Unit]                    = stg.deleteByKey(_.fcid == fcid)
  def get(ftid: Ftid): IO[DataStoreErr, Option[FlinkTmMetrics]] = stg.get(ftid)
  def list(fcid: Fcid): IO[DataStoreErr, List[FlinkTmMetrics]]  = stg.getByKey(_.fcid == fcid)
  def listTmId(fcid: Fcid): IO[DataStoreErr, List[Ftid]]        = stg.getKeys.map(_.filter(_.fcid == fcid))
