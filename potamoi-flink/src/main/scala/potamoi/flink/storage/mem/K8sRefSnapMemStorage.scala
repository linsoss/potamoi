...
object K8sRefSnapMemStorage:
  ...
  private object DeploySnapStg extends LWWMapDData[(Fcid, String), FlinkK8sDeploymentSnap]("flink-k8s-deploy-snap-stg"):
    def apply(): Behavior[Req] = behavior(DDataConf.default)

  private object SvcSnapStg extends LWWMapDData[(Fcid, String), FlinkK8sServiceSnap]("flink-k8s-svc-snap-stg"):
    def apply(): Behavior[Req] = behavior(DDataConf.default)

  private object PodSnapStg extends LWWMapDData[(Fcid, String), FlinkK8sPodSnap]("flink-k8s-pod-snap-stg"):
    def apply(): Behavior[Req] = behavior(DDataConf.default)

  private object PodMetricsStg extends LWWMapDData[(Fcid, String), FlinkK8sPodMetrics]("flink-k8s-pod-metrics-stg"):
    def apply(): Behavior[Req] = behavior(DDataConf.default)

  private object ConfigmapNamesStg extends ORSetDData[(Fcid, String)]("flink-k8s-configmap-names-stg"):
    def apply(): Behavior[Req] = behavior(DDataConf.default)
  ...
  def make: ZIO[ActorCradle, Throwable, K8sRefSnapStorage] = for {
    ...
    val deployment: K8sDeploymentSnapStorage = new K8sDeploymentSnapStorage:
      ...
      def rm(fcid: Fcid): DIO[Unit]                                                = deploySnapStg.removeBySelectKey(_._1 == fcid).uop
      ...
      def listName(fcid: Fcid): DIO[List[String]]                                  = deploySnapStg.listKeys().map(_.map(_._2)).rop
    ...
    val service: K8sServiceSnapStorage = new K8sServiceSnapStorage:
      ...
      def rm(fcid: Fcid): DIO[Unit]                                          = svcSnapStg.removeBySelectKey(_._1 == fcid).uop
      ...
      def listName(fcid: Fcid): DIO[List[String]]                            = svcSnapStg.listKeys().map(_.map(_._2)).rop
    ...
    val pod: K8sPodSnapStorage = new K8sPodSnapStorage:
      ...
      def rm(fcid: Fcid): DIO[Unit]                                      = podSnapStg.removeBySelectKey(_._1 == fcid).uop
      ...
      def listName(fcid: Fcid): DIO[List[String]]                        = podSnapStg.listKeys().map(_.map(_._2)).rop
    ...
    val podMetrics: K8sPodMetricsStorage = new K8sPodMetricsStorage:
      ...
      def rm(fcid: Fcid): DIO[Unit]                                         = podMetricsStg.removeBySelectKey(_._1 == fcid).uop
      ...
      def listName(fcid: Fcid): DIO[List[String]]                           = podMetricsStg.listKeys().map(_.map(_._2)).rop
    ...
    val configmap: K8sConfigmapNamesStorage = new K8sConfigmapNamesStorage:
      ...
      def rm(fcid: Fcid): DIO[Unit]                         = configmapStg.list().map(_.filter(_._1 == fcid)).flatMap(configmapStg.removes(_)).uop
      def listName(fcid: Fcid): DIO[List[String]]           = configmapStg.list().map(_.map(_._2).toList).rop
  }
