...
class FlinkRestRequest(restUrl: String) {
   ...
   */
  def listTaskManagerIds: IO[FlinkRestErr, Vector[String]] = usingSttp { backend =>
    request
   ...
   */
  def getTaskManagerDetail(tmId: String): IO[FlinkRestErr, FlinkTmDetail] = usingSttp { backend =>
    request
   ...
   */
  def getTmMetrics(tmId: String, metricsKeys: Set[String]): IO[FlinkRestErr, Map[String, String]] = usingSttp { backend =>
    request
   ...
   */
  def getTmMetricsKeys(tmId: String): IO[FlinkRestErr, Set[String]] = usingSttp { backend =>
    request
